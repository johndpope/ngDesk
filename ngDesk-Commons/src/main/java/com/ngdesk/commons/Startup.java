...
public class Startup implements ApplicationListener<ApplicationReadyEvent> {
	...
	public void onApplicationEvent(ApplicationReadyEvent event) {
		try {
			String[] timezones = { "Africa/Abidjan", "Africa/Accra", "Africa/Addis_Ababa", "Africa/Algiers",
			...
			// Load Error Files
			Global.translations = new HashMap<String, Map<String, String>>();
			String languages[] = { "en" };
			for (String language : languages) {
				String translationsJson = global.getFile("translations/" + language + ".json");
				String translationsCommonJson = global.getFile("translations/" + language + "-commons.json");

				Map<String, String> translationsMap = new ObjectMapper().readValue(translationsJson, Map.class);
				Map<String, String> commonTranslationsMap = new ObjectMapper().readValue(translationsCommonJson,
						Map.class);

				Map<String, String> combinedMap = new HashMap<String, String>();
				combinedMap.putAll(translationsMap);
				combinedMap.putAll(commonTranslationsMap);

				Global.translations.put(language, combinedMap);
			}

			String currencies = global.getFile("currencies.json");
			ObjectMapper mapper = new ObjectMapper();
			Global.currencies = mapper.readValue(currencies, HashMap.class);

			HashMap<String, String> updatedCurrenciesMap = new HashMap<String, String>();
			for (String key : Global.currencies.keySet()) {
				updatedCurrenciesMap.put(Global.currencies.get(key), key);
			}

			Global.currenciesReverseLookUp = updatedCurrenciesMap;

			// SETTING TIMEZONES
			Global.timezones = Arrays.asList(timezones);
