...
public class OnCallUser {
	...
	public String CallUser(String name, String companyId) {
		...
		try {
			...
			String scheduleId = scheduleDocument.remove("_id").toString();
			
			Schedule schedule = mapper.readValue(mapper.writeValueAsString(scheduleDocument), Schedule.class);
			...
			for (int i = layers.size() - 1; i >= 0; i--) {
				...
				String rotationType = layer.getRotationType();
				List<String> users = layer.getUsers();

				Date parsedDate = layer.getStartDate();
				...
				if (rotationType.equals("Weekly")) {
					...
					userId = users.get((noOfRotations - 1) % users.size()).toString();
				...
				// Restrictions
				boolean hasRestrictions = layer.isHasRestrictions();
				if (hasRestrictions) {
					List<Restriction> layerRestrictions = layer.getRestrictions();
					for (int j = 0; j < layerRestrictions.size(); j++) {
						Restriction restriction = layerRestrictions.get(j);
						String restrictionType = layer.getRestrictionType();
