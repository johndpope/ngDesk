...
public class ElasticSearchForDiscoveryMaps {
	...
	public List<ObjectId> getObjectIdsFromElastic(String searchString, String companyId) {
		...
		try {
			...
			for (SearchHit searchHit : searchHits) {
				...
				ids.add(new ObjectId(discoveryId));
			}
	...
	public void deleteDiscoveryMapsToElastic(DiscoveryMap discoveryMap) {
		try {
			BoolQueryBuilder queryBuilder = new BoolQueryBuilder();
			queryBuilder.must().add(QueryBuilders.matchQuery("id", discoveryMap.getId()));
			queryBuilder.must().add(QueryBuilders.matchQuery("companyId", discoveryMap.getCompanyId()));

			DeleteByQueryRequest deleteByQueryRequest = new DeleteByQueryRequest("sam_discovery_map");
