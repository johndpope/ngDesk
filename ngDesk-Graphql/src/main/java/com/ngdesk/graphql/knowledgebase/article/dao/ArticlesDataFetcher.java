...
public class ArticlesDataFetcher implements DataFetcher<List<Article>> {
	...
	public List<Article> get(DataFetchingEnvironment environment) throws Exception {
		...
		if (search != null) {
			List<ObjectId> ids = articleService.getIdsFromElastic(companyId, search, teams, isSystemAdmin);
			return articleRepository.findAllWithPageableAndSearch(pageable, ids, collectionName);
		} else {
			if (isSystemAdmin) {
				return articleRepository.findAllWithPageable(pageable, collectionName);
			} else {
				return articleRepository.findAllWithPageableAndTeam(teams, pageable, collectionName);
			}
		}
