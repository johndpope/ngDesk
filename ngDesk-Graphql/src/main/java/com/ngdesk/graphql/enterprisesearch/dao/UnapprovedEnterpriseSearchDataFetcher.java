...
@Component
public class UnapprovedEnterpriseSearchDataFetcher implements DataFetcher<List<EnterpriseSearch>> {
	@Autowired
	...
	public List<EnterpriseSearch> get(DataFetchingEnvironment environment) {
		...
		Optional<List<EnterpriseSearch>> optionalEnterpriseSearch = enterpriseSearchRepository
				.findAllUnapprovedEnterpriseSearch(pageable, "Enterprise_Search");
		if (optionalEnterpriseSearch.isPresent()) {
			return optionalEnterpriseSearch.get();
		}
		return null;
	}
