...
export class FormsRelationshipService {
	...
	public getRelationship(field, layoutStyle) {
		...
		if (
			...
			return `<mat-form-field id = "${field.NAME.toLowerCase()}-dropdown" appearance="${layoutStyle}" floatLabel="always" fxFlex >
        ...
        <input #${field.NAME}Input type="text"
         ...
         [matAutocomplete]="${
      	...
      	(blur)="context.clearInput(${field.NAME}Input, '${field.NAME}')">
        ...
        (optionSelected)="context.addDataForRelationshipField(context.fieldsMap['${
      	...
      	<mat-option *ngFor="let entry of context.customModulesService.relationFieldFilteredEntries['${
					field.NAME
				}']"
      	   [value]="entry"> {{entry['PRIMARY_DISPLAY_FIELD']}} 
         </mat-option>
        </mat-autocomplete>
        <mat-icon matSuffix style="cursor: pointer;display: inline-block;" matTooltip="Create entry for ${
					field.DISPLAY_LABEL
				}" 
        ...
        <mat-hint>${field.HELP_TEXT}</mat-hint>
			...
			return `<mat-form-field appearance="${layoutStyle}" floatLabel="always" fxFlex >
      			<mat-label>${field.DISPLAY_LABEL}</mat-label>
      	<mat-chip-list  [disabled]="!context.formEnabled || context.fieldsMap['${field.FIELD_ID}'].NOT_EDITABLE" #${field.NAME}ChipList
       ...
       (appAutocompleteScroll)="context.customModulesService.onScroll(context.fieldsMap['${field.FIELD_ID}'])" 
        #${field.NAME}_auto="matAutocomplete"
          ...
          <mat-hint>${field.HELP_TEXT}</mat-hint>
			...
			return `<div style="padding-bottom: 15px;" fxFlex="100" fxLayout="column" fxLayoutGap="10px">
          <div>
            <mat-expansion-panel [expanded]="context.customModulesService.oneToManyControls['FIELD_IN_FOCUS'] === '${field.FIELD_ID}'" (click)="context.customModulesService.customTableService.isLoading = true;context.customModulesService.oneToManyControls['FIELD_IN_FOCUS'] = '${field.FIELD_ID}';context.customModulesService.setupOneToManyTable(context.module, '${field.MODULE}', context.fieldsMap['${field.FIELD_ID}'], context.entry)">
               ...
               <mat-panel-title fxFlex="95">
                ...
                <mat-panel-description fxLayoutAlign="center center">
                  <button style="margin-right:'20px" mat-button
                    (click)="context.openOneToManyMapDialog('${field.MODULE}', context.fieldsMap['${field.FIELD_ID}']) ; $event.stopPropagation();">
                  ...
                  <button mat-button
                    ...
                    <mat-icon matTooltip="Create and map ${field.DISPLAY_LABEL} to this {{context.module.NAME}}">add_circle_outline</mat-icon>
                  </button>
                </mat-panel-description>
              </mat-expansion-panel-header>
                        <div style="width: 100%;">
                    <div fxLayout="column" class="mat-elevation-z8" [ngStyle]="context.customModulesService.customTableService.totalRecords> 0 && !context.customModulesService.customTableService.isLoading?{'display': 'block'} : {'display': 'none'}">
                        <table mat-table
                            *ngIf="!context.customModulesService.customTableService.isLoading  && context.customModulesService.customTableService.totalRecords > 0"
                            ...
                            (matSortChange)="context.customModulesService.tableOperationClicked = true;context.customModulesService.onPageChange($event, '${field.MODULE}', context.fieldsMap['${field.FIELD_ID}']);"
                            ...
                            <ng-container matColumnDef="{{col.DISPLAY}}"
                                *ngFor="let col of context.customModulesService.customTableService.columnsHeadersObj ">
                                ...
                                <td mat-cell *matCellDef="let element" class="pointer">
                                    ...
                                    <span
                                        *ngIf="col.DATA_TYPE !== 'Button' && col.DATA_TYPE !== 'Date/Time' && col.DATA_TYPE !== 'Relationship' && col.DATA_TYPE !== 'Date' &&  col.DATA_TYPE !== 'Time'&& col.DATA_TYPE !== 'Number' && col.DATA_TYPE !=='Text' && col.DATA_TYPE !=='Text Area'">{{element[col.NAME]}}</span>
                            ...
                            <tr mat-row *matRowDef="let row; columns: context.customModulesService.customTableService.columnsHeaders"
                                (click)="context.openEditForOneToManyCreateLayout(row, context.fieldsMap['${field.FIELD_ID}'])"></tr>
                        </table>
                        <mat-paginator [ngStyle]="context.customModulesService.customTableService.totalRecords> 0 && !context.customModulesService.customTableService.isLoading?{'display': 'block'} : {'display': 'none'}"  (page)="context.customModulesService.tableOperationClicked = true; context.customModulesService.onPageChange($event, '${field.MODULE}', context.fieldsMap['${field.FIELD_ID}'])"
