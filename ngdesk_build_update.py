...
def start_containers(image_path, image_name):
    ...
    #     client.containers.run(image_path, name=image_name, detach=True, network_mode='host', environment=['ELASTICSEARCH_HOSTS=http://localhost:9200'])
        ...
        client.containers.run(image_path, name=image_name, detach=True, network_mode='host', volumes={'/ngdesk/nginx/exmaple.crt':{'bind':'/etc/nginx/keys/exmaple.crt'}, '/ngdesk/nginx/exmaple.key':{'bind':'/etc/nginx/keys/exmaple.key'}})
